...
| Closing statement band | `components/sections/Closing.tsx` | `closing` |
| Footer | `components/layout/Footer.tsx` | `footer`, `company` |
...
(dropped with the dark-first hero/showcase glow treatment). Each has a
...
- `contact` — section title/body, the `mailto:` fallback address, and
  `interestOptions` (still lists Traveller/Chauffeur/Partner/Investor/
  Careers/Other — this is how the old four-audience-card segmentation is
  preserved after the homepage collapsed them into one Partner banner).
